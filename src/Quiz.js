...
React.useEffect(async() =>{
    ...
    const questionsList = questionData.map(quest =>{
        ...
        return (
            ...
            <p>{quest.question.replace(/&quot;/g, '\"').replace(/&#039;/g, "\'")}</p>
            {optionList}
            <br/><hr/>
            </div>
        );
    ...
    return(
        ...
        {submitButton && !isSubmit && questionsList}
        {submitButton && !isSubmit && !ansShown && <button onClick={checkAnswer} className="submit-button">Submit Test</button>}
